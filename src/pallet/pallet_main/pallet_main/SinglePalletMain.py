...
class TurtlePControl(Node):
    def __init__(self):
        super().__init__('SinglePalletMain')
        client_group1=ReentrantCallbackGroup()
        server_group=client_group1
        self.mainsrv = self.create_service(Maincontroller, 'SinglePallet', self.actived_callback,callback_group=server_group)        # CHANGE
        ...
        self.ready_get_endpose_publisher = self.create_publisher(Int64,'/Pallet/single/endposeready',10,callback_group = client_group1)
        # # 手臂命令
        # self.arm_pose_publisher = self.create_publisher(Twist, '/arm_control', 10)   #topic和msg 名稱要改！！！
        self.real_startpose_subscriber = self.create_subscription(Twist, '/Pallet/single/realstartpose', self.startpose, 10,callback_group = client_group1)
        self.real_endpose_subscriber = self.create_subscription(Twist, '/Pallet/single/realendpose', self.endpose, 10,callback_group = client_group1)

        # # YOLO 命令
        self.yolo_cmd_publisher = self.create_publisher(Int64, '/Pallet/yolo_cmd', 10,callback_group = client_group1)  
        self.stack_full_sub = self.create_subscription(Int64, '/Pallet/single/multistackfull',self.stack_full, 10,callback_group = client_group1)
        ...
        self.photopose=[-696.53,-20.11,618.61,180.0,0.0,-90.0]
        self.convergetpose=[-696.53,-20.11,0.0,180.0,0.0,-90.0]
    ...
    def startpose(self, msg):
        ...
        self.opensuck()
        inner_req=self.get_movepose_req([a + b + c for a, b, c in zip(self.convergetpose, startpose, [0.0,0.0,100.0,0.0,0.0,0.0])])
        future = self.maniclient.call_async(inner_req)
        while not future.done():
            time.sleep(0.01)
        try:
        ...
        try:
            ...
            self.get_logger().error(f"start_pose Inner service failed: {e}")
            aa = -1
        inner_req=self.get_movepose_req([a + b + c for a, b, c in zip(self.convergetpose, startpose, [0.0,0.0,400.0,0.0,0.0,0.0])])
        future = self.maniclient.call_async(inner_req)
        while not future.done():
            time.sleep(0.01)
        try:
