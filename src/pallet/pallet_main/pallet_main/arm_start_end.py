...
class YoloCmdNode(Node):
    def __init__(self):
        ...
        self.base_pose = [0.0,550.0,500.0,180.0,0.0,0.0]  # 預設基座位姿
        self.eyetohand=[0.0,100.0,150.0,0.0,0.0,0.0]
    def loop(self):
        if self.sensor_flag:
            msg=Int64()
    ...
    def startpose(self, msg):
        ...
        real_startpose = Twist()
        real_startpose.linear.x = msg.linear.x + self.base_pose[0]
        real_startpose.linear.y = msg.linear.y + self.base_pose[1]
        real_startpose.linear.z = msg.linear.z + self.base_pose[2] 
        real_startpose.angular.x = msg.angular.x + self.base_pose[3]
        real_startpose.angular.y = msg.angular.y + self.base_pose[4]
        real_startpose.angular.z = msg.angular.z + self.base_pose[5]
        ...
        self.real_startpose_pub.publish(real_startpose)
