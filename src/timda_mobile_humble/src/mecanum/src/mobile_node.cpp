...
#include "tf2_geometry_msgs/tf2_geometry_msgs.hpp"  // 包含 tf2_geometry_msgs 头文件
...
class TimdaMobile : public rclcpp::Node
{
    // ros1
    // ros::NodeHandle nh_;: pub: public rclcpp::Nodelic rclcpp::Nod: public rclcpp::Nodee
    // ros::Subscriber sub_;
...
public:
    TimdaMobile() : Node("mobile_node")
    {
        ...
        sub_ = this->create_subscription<geometry_msgs::msg::Twist>("cmd_vel", 10, std::bind(&TimdaMobile::Callback, this, std::placeholders::_1));
        odom_pub = this->create_publisher<nav_msgs::msg::Odometry>("odom", 50);
        loop_rate = std::make_shared<rclcpp::Rate>(1000);
    ...
    void Looper()
    {
        while (rclcpp::ok()) {
            ...
            geometry_msgs::msg::Quaternion odom_quat = tf2::toMsg(tf2::Quaternion(tf2::Vector3(0, 0, 1), rz));
...
//   ros::init(argc, argv, "mobile_node");
  ...
  printf("AA");
//   rclcpp::spin(std::make_shared<TimdaMobile>());
  printf("AA");
  TimdaMobile tm;
